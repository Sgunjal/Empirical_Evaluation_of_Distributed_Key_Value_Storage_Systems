...
public class Client1_Test_P extends Thread
{
	String type;
	...
	int start_counter=100000;
	int end_counter=200000;
	int port1=25001;
	int thread_start=0;
	int thread_end=3;
	...
	int k=0;
	Client1_Test_P(String type)
	{
		this.type=type;
	}
	Client1_Test_P(Socket s,String type)
	{
		this.type=type;
		incoming_socket=s;
	}
	...
	{		
		try
		{
			if(type.equals("Server"))
			{
				...
				{
					 Socket s = ss.accept();  		
					 Thread t1 =new Client1_Test_P(s,"Request");		
					 t1.start();
				}				
			}
			else
	...
	synchronized public void Client_Choice(BufferedReader sc_input1,Socket[] s2_array,BufferedReader[] s_input2_array,PrintWriter[] s_out2_array)
	{
		...
		int choice;
		String repeat_menu;		
		System.out.println("1: Start Operations");
		...
		switch(choice)
		{
			case 1: generate_key_value();
					...
					client_get(s2_array,s_input2_array,s_out2_array);
					client_delete(s2_array,s_input2_array,s_out2_array);
	...
	void generate_key_value()
	{
		...
		{		
			temp="";
			i_key[k]= Integer.toString(key_start_c) ;
			i_val[k]=Integer.toString(key_start_c);
			for(z=1;z<(10-(i_key[k].length()));z++)
			{
				temp=temp+"k";				
			}
			i_key[k]=i_key[k]+temp;
			temp="";	
			for(z=1;z<(90-(i_val[k].length()));z++)
			{
				temp=temp+"k";				
			}
			i_val[k]=i_val[k]+temp;
			key_start_c++;
	 ...
	 public static void main(String args[]) throws Exception
	{
		...
		{			
			...
			Client1_Test_P ct=new Client1_Test_P("Client");
			Thread t=new Client1_Test_P("Server");		
			t.start();
			ct.client_side();
		}
		catch(Exception e)
